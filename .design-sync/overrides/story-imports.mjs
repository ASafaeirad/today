...
// FORK REASON (this repo only): the stories are CSF *factories* -
// `definePreview(...)` -> `preview.meta({component})` -> `meta.story({...})`,
// with no default export. The upstream inert proxy can't serve that: esbuild's
// CJS interop copies named imports from OWN enumerable props, so
// `import { definePreview }` resolved to undefined and every preview died with
// "definePreview is not a function". Even reached through the proxy, an inert
// `meta.story()` returns a callable placeholder, not a story object.
//
// So implement the factory chain for real, in the terms the generated wrapper's
// compose() already understands: each `story()` returns a PLAIN CSF object with
// preview -> meta -> story precedence already folded in (args/argTypes/
// parameters merged, decorators outermost-first, render/component inherited).
// compose() reads `S.default` for meta, which CSF factories don't export -
// pre-merging here is what keeps meta-level config from being dropped.
const CSF_FACTORY_STUB =
  'function dsMerge(a,b){return{' +
  'component:(b&&b.component)!==void 0?b.component:(a&&a.component),' +
  'render:(b&&b.render)!==void 0?b.render:(a&&a.render),' +
  'args:Object.assign({},a&&a.args,b&&b.args),' +
  'argTypes:Object.assign({},a&&a.argTypes,b&&b.argTypes),' +
  'parameters:Object.assign({},a&&a.parameters,b&&b.parameters),' +
  'decorators:[].concat((b&&b.decorators)||[]).concat((a&&a.decorators)||[])};}' +
  'function dsStory(base,si){var s=dsMerge(base,si);s.story=function(x){return dsStory(s,x)};return s}' +
  'function dsMeta(pv,mi){var mt=dsMerge(pv,mi);mt.story=function(si){return dsStory(mt,si)};return mt}' +
  'function dsDefinePreview(p){var pv=dsMerge(null,p);pv.meta=function(mi){return dsMeta(pv,mi)};return pv}' +
  'm.definePreview=dsDefinePreview;m.__definePreview=dsDefinePreview;';
